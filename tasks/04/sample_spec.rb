describe Card do
  it 'can be converted to a string' do
    ...
    expect(Card.new(:ace, :diamonds).to_s).to eq 'Ace of Diamonds'
  end
  it 'can be checked if equal' do
    expect(Card.new(10, :spades) == Card.new(10, :spades)).to eq true
    expect(Card.new(:ace, :diamonds) == Card.new(10, :spades)).to eq false
  end
end
...
describe SixtySixDeck do
  ...
  describe 'hand' do
    ...
    it "Checks the hand for twenty when it's present" do
      ...
      hand = SixtySixDeck.new(deck).deal

      expect(hand.twenty?(:diamonds)).to be true
    end

    it "Checks the hand for twenty when it's not present" do
      deck = [Card.new(:ace, :spades), Card.new(:king, :spades),
              Card.new(9, :spades), Card.new(9, :hearts),
      ...
      expect(hand.twenty?(:diamonds)).to be false
    end

    it "Checks the hand for forty when it's present" do
      deck = [Card.new(:ace, :spades), Card.new(:king, :spades),
              Card.new(9, :spades), Card.new(9, :hearts),
              Card.new(:king, :diamonds), Card.new(:queen, :spades)
             ]
      ...
      expect(hand.forty?(:spades)).to be true
    end

    it "Checks the hand for forty when it's not present" do
      deck = [Card.new(:ace, :spades), Card.new(:king, :spades),
              Card.new(9, :spades), Card.new(9, :hearts),
              Card.new(:king, :diamonds), Card.new(:jack, :spades)
             ]
      ...
      expect(hand.forty?(:spades)).to be false
    end
  end
end
